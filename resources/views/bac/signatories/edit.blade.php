@section('title', 'Edit BAC Signatory')

<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-2xl text-gray-800 leading-tight">{{ __('Edit BAC Signatory') }}</h2>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('bac.signatories.update', $signatory) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="flex space-x-4">
                                <label class="inline-flex items-center">
                                    <input type="radio" name="input_type" value="user" 
                                           {{ old('input_type', $signatory->user_id ? 'user' : 'manual') == 'user' ? 'checked' : '' }}
                                ...
                                <label class="inline-flex items-center">
                                    <input type="radio" name="input_type" value="manual" 
                                           {{ old('input_type', $signatory->user_id ? 'user' : 'manual') == 'manual' ? 'checked' : '' }}
                                           class="form-radio text-cagsu-blue" 
                        ...
                        <!-- User Selection -->
                        <div id="user-section" class="{{ old('input_type', $signatory->user_id ? 'user' : 'manual') == 'user' ? '' : 'hidden' }}">
                            ...
                            <select name="user_id" id="user_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-cagsu-blue focus:ring focus:ring-cagsu-blue focus:ring-opacity-50">
                                ...
                                @foreach($bacUsers as $user)
                                    <option value="{{ $user->id }}" {{ old('user_id', $signatory->user_id) == $user->id ? 'selected' : '' }}>
                        ...
                        <!-- Manual Name Entry -->
                        <div id="manual-section" class="{{ old('input_type', $signatory->user_id ? 'user' : 'manual') == 'manual' ? '' : 'hidden' }}">
                            <label for="manual_name" class="block text-sm font-medium text-gray-700">Full Name <span class="text-red-500">*</span></label>
                            <input type="text" name="manual_name" id="manual_name" value="{{ old('manual_name', $signatory->manual_name) }}" 
                        ...
                        <div>
                            ...
                            <select name="position" id="position" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-cagsu-blue focus:ring focus:ring-cagsu-blue focus:ring-opacity-50" required>
                                ...
                                <option value="bac_chairman" {{ old('position', $signatory->position) == 'bac_chairman' ? 'selected' : '' }}>BAC Chairman</option>
                                <option value="bac_vice_chairman" {{ old('position', $signatory->position) == 'bac_vice_chairman' ? 'selected' : '' }}>BAC Vice Chairman</option>
                                <option value="bac_member" {{ old('position', $signatory->position) == 'bac_member' ? 'selected' : '' }}>BAC Member</option>
                                <option value="head_bac_secretariat" {{ old('position', $signatory->position) == 'head_bac_secretariat' ? 'selected' : '' }}>Head, BAC Secretariat</option>
                                <option value="ceo" {{ old('position', $signatory->position) == 'ceo' ? 'selected' : '' }}>CEO</option>
                                <option value="canvassing_officer" {{ old('position', $signatory->position) == 'canvassing_officer' ? 'selected' : '' }}>Canvassing Officer</option>
                            </select>
                            @error('position')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="prefix" id="prefix" value="{{ old('prefix', $signatory->prefix) }}" placeholder="e.g., Dr., Atty., Engr." class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-cagsu-blue focus:ring focus:ring-cagsu-blue focus:ring-opacity-50">
                            ...
                            <div>
                                ...
                                <input type="text" name="suffix" id="suffix" value="{{ old('suffix', $signatory->suffix) }}" placeholder="e.g., Ph.D., M.A., CPA" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-cagsu-blue focus:ring focus:ring-cagsu-blue focus:ring-opacity-50">
                        ...
                        <div>
                            <label class="flex items-center">
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $signatory->is_active) ? 'checked' : '' }} class="rounded border-gray-300 text-cagsu-blue shadow-sm focus:border-cagsu-blue focus:ring focus:ring-cagsu-blue focus:ring-opacity-50">
