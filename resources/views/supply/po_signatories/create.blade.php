@section('title', 'Add PO Signatory')

<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center gap-4">
            <h2 class="font-semibold text-2xl text-gray-800 leading-tight">{{ __('Add PO Signatory') }}</h2>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('supply.po-signatories.store') }}" method="POST" class="space-y-6">
                        @csrf
                        ...
                        <div>
                            ...
                            <select name="position" id="position" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-cagsu-blue focus:ring focus:ring-cagsu-blue focus:ring-opacity-50" required>
                                ...
                                <option value="chief_accountant" {{ old('position') == 'chief_accountant' ? 'selected' : '' }}>Chief Accountant</option>
                            </select>
                            @error('position')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
